...
#### 0002 — do not track scroll events as pointers

[`third_party/0002-io-input-do-not-track-scroll-events-as-pointers.patch`](third_party/0002-io-input-do-not-track-scroll-events-as-pointers.patch). Submitted upstream at [lists.sr.ht/~eliasnaur/gio-patches/patches/69233](https://lists.sr.ht/~eliasnaur/gio-patches/patches/69233).

**Why:** scroll events arrived at `pointerQueue.Push` and went through `pointerOf` + `deliverEnterLeaveEvents` + `deliverEvent` like Move/Press/Release, so each scroll created or updated a `state.pointers` entry, populated `p.entered` with handlers under the wheel position, and overwrote `state.cursor` based on hit-test at the scroll position. On Windows the backend reported scroll with `PointerID=0` (`scrollEvent` omitted the field) while `pointerUpdate` forwarded Windows' assigned ID (typically 1+) for the same physical mouse, so the scroll spawned a phantom `state.pointers` entry. Subsequent moves went to the mouse's "real" entry, the phantom never received `Leave`, and `pointerQueue.Frame` ran hit-test for it every frame at the user's last scroll position — clobbering the live pointer's cursor resolution. Hovering an anchor badge correctly resolved `CursorPointer` for the live pointer, then the phantom (sitting over an editor cell) overwrote it with `CursorText` for the same frame. Same mechanism produced the "two rows hovered at once" symptom because the phantom's `entered` tags never released.

**Patch:** the wheel is positional but isn't a pointer — treat scroll events as disembodied. Hit-test inline at the scroll position to find delivery targets, dispatch via the existing `deliverEvent` (which already handles filter matching, scroll-axis clamping, and area-local position), and return without creating or updating a `state.pointers` entry.
